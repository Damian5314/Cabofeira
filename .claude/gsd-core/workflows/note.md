...
- **Global scope**: `E:/Projecten/Cabofeira/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `E:/Projecten/Cabofeira/.claude/notes/`)
...
2. Glob `E:/Projecten/Cabofeira/.claude/notes/*.md` (if directory exists) — global notes
...
Global (E:/Projecten/Cabofeira/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `E:/Projecten/Cabofeira/.claude/notes/` — works in any directory
